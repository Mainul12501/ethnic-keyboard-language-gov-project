...
function createAudioRow(arr) {
	...
	for(var i in arr){
		...
		if(i==0) {
			...
			tableData.setAttribute("type", "radio");
			tableData.setAttribute("style", "margin:1rem; cursor:pointer;");
			tableData.setAttribute("name", "checkbox");
			tableData.setAttribute("data-id", arr[i]);
            tableData.setAttribute("onClick", "downloadTrack('"+arr[i].toString()+"')");
			tableData.setAttribute("data-checkbox", 'true');
			tableData.setAttribute("class", "w3-check w3-margin-left default-class getData-"+arr[0]+i);
		} else {
			tableData = document.createElement("td");
            tableData.setAttribute("class",'time'+arr[0]+i);
			tableData.innerText = arr[i].toFixed(4);
		}
		tableData.setAttribute("id",arr[0]+i);
		tableData.setAttribute("class",'default-class default-row-'+arr[0]+i);
